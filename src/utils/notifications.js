...
console.log("📱 [Notifications] SMS_API_KEY:", process.env.SMS_API_KEY && process.env.SMS_API_KEY !== "your_fast2sms_api_key_here" ? "✅ SET" : "❌ NOT SET / placeholder");
...
function getTransporter() {
  ...
  const missingOrPlaceholder =
    !nodemailer ||
    !smtpHost || smtpHost.includes("placeholder") ||
    !smtpUser || smtpUser === "your_gmail@gmail.com" || smtpUser.includes("placeholder") ||
    !smtpPass || smtpPass === "your_16_char_app_password" || smtpPass.includes("placeholder");
...
async function sendSMSNotification(phone, message) {
  ...
  try {
    const sendFn = fetch("https://www.fast2sms.com/dev/bulkV2", {
      method: "POST",
      headers: {
      ...
      throw new Error(data.message || "Fast2SMS returned error");
    });
